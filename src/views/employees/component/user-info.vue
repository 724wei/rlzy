  ...
  <div class="user-info">
    <el-row type="flex" justify="end">
      <el-tooltip class="item" effect="dark" content="打印个人信息" placement="bottom-end">
        <router-link :to="`/employees/print/${userId}?type=personal`"><i class="el-icon-printer" /></router-link>
      </el-tooltip>
    </el-row>
    <!-- 个人信息 -->
...
import { getPersonalDetail, saveUserDetailById, updatePersonal } from '@/api/employee'
...
export default {
  ...
  methods: {
    async saveUser() {
      const fileList = this.$refs.avatarImg.fileList
      console.log(this.$refs.avatarImg.fileList)
      if (fileList.some(item => !item.upload)) {
        this.$message.warning('上传未完成')
        return
      }
